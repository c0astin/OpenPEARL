 ...
 * 
 * The names are marked with a '$' symbol. Each name is replaced by the value, which is taken from
...
public class EvaluateNamedExpression {
    ...
    /**
     * substitute names by their current values and evaluate the resulting expression as integer. The
     * names are prefixed with '$' and may consist only of upper and lower case letters. Expressions
     * must be braced with square brackets. e.g. 'anyText[1+4*$name + $name]anyotherText'
     * 
     * multiple expressions are allowed
     * 
     * @param expr the expression with names and operators
    ...
    public String evaluateExpression(String expr) {
        ...
        while (nameStart >= 0) {
            ...
            expanded = expr.substring(0, nameStart) + nameValue + expr.substring(nameEnd);
        ...
        while (exprStart >= 0) {
            ...
            try {
                ...
                Log.internalError(
                        "wrong expression in target plattform definition file (" + nakedExpression + ")");
                return null;
            }

            // insert expression result in the expanded string
            expanded = expanded.substring(0, exprStart) + result + expanded.substring(exprEnd + 1);
        ...
        static int getFactor() {
            ...
            if (skipWhitespaceAndTakeIfPresent('(')) {
                ...
                throw new RuntimeException(
                        "unexpected character in expression: " + (char) currentCharacter);
            }
