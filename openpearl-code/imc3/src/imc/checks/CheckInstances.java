 ...
 * instances="1" instances="oncePerSet"
 * 
 * @author mueller
 *
 */
                ...
                if (nl.item(i).getNodeName().equals("check")) {
                    ...
                    if (nodeOfCheck != null) {
                        if (nodeOfCheck.getTextContent().equals("oncePerSet")) {
                            ...
                            if (nodeOfSet == null) {
                                Log.error("internal error: missing attribute set for oncePerSet in "
                                        + pse.getSystemName());
                                return;
                            }
                            ...
                            if (pse.getUsedBy().size() > 1) {
                                for (int j = 0; j < pse.getUsedBy().size() - 1; j++) {
                                    ...
                                    EvaluateNamedExpression expr1 =
                                            new EvaluateNamedExpression(pse.getUsedBy().get(j).getParameters());
                                    String sj = expr1.evaluateExpression(set);
                                    for (int k = j + 1; k < pse.getUsedBy().size(); k++) {
                                        EvaluateNamedExpression expr2 =
                                                new EvaluateNamedExpression(pse.getUsedBy().get(k).getParameters());
                                        String sk = expr2.evaluateExpression(set);
                                        if (sj.equals(sk)) {
                                            Log.setLocation(getModuleOf(pse.getUsedBy().get(k)).getSourceFileName(),
                                                    pse.getUsedBy().get(k).getLine(), pse.getUsedBy().get(k).getCol());
                                    ...
                                    if (errorPrinted) {
                                        Log.note(getModuleOf(pse.getUsedBy().get(j)).getSourceFileName(),
                                                pse.getUsedBy().get(j).getLine(), pse.getUsedBy().get(j).getCol(),
                            ...
                            Log.internalError("unknown kind of instances check in " + pse.getSystemName()
                            + "  check: " + nodeOfCheck.getTextContent());
                            return;
                        }
                    }
                }
