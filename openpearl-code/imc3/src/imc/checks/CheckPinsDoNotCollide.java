 ...
 * 
 * Run thru all ModuleSystemEntry and check if their PlatformEntry contains a check with name
 * pinDoesNotCollide. A pin is a representation of a physical i/o pin of a device. The number are
 * choosen usually according the logic function. The name pin may represent a digital i/o bit as
 * well as a multiplexer channel of a adc device.
 * 
 * The xml tag looks like
 * <check pinDoesNotCollide="PCF8574xx(I2cAdr=$iicAdr)" start="$start" width= "$width" /> The
 * attribute of the pinDoesNotCollide defines a so called 'PinProvider'. If the platform device
 * needs an associationProvider, the concrete username of the association provider is added to the
 * given provider string.
 * 
 * Each PinProvider maintains a list of all used bits and the using ModuleEntrySystemPart.
 * 
 * After collecting all elements of a module, the list for each PinProvider is checked for duplicate
 * pin numbers.
 * 
 * Note that in the PinProvider definition the same parametric expressions are allowed with the
 * named parameters of the platform element. By this way PCF8574xx(I2cAdr=$iicAdr)" will be
 * substituted into "PCF8574xx(I2cAdr='40'B4)"
 * 
 * @author mueller
 *
 */
...
public class CheckPinsDoNotCollide {
    ...
    class PinProvider {
        ...
        /**
         * mark a range of pins as used; this is useful for digital i/o, when several bits are used as
         * i/o device for bit strings
         * 
         * @param start starting number of the pin range; note numbering starts with the most
         *        significant pin/bit
        ...
        for (Module m : modules) {
            for (ModuleEntrySystemPart se : m.getSystemElements()) {
                ...
                for (int i = 0; i < nl.getLength(); i++) {
                    ...
                    String provider =
                            nl.item(i).getAttributes().getNamedItem("pinDoesNotCollide").getTextContent();
                    ...
                    // System.out.println(sysname+" provider: " + provider + " pins needed "+start+" "+
                    // width);
            ...
            for (PinProvider pp : pinProviders) {
                ...
                for (int i = 0; i < usedBits - 1; i++) {
                    for (int j = i + 1; j < usedBits; j++) {
                        if (pp.usedBits.get(i).pin == pp.usedBits.get(j).pin) {
                            ...
                            Log.error("'" + definitionFromUsername(mse2) + " " + errorMsg + " "
                                    + pp.usedBits.get(j).pin + " already in use");
                            if (mse1 != null) {
