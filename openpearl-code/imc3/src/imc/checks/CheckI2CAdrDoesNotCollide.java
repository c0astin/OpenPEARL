    ...
    public CheckI2CAdrDoesNotCollide(List<Module> modules) {
        for (Module m : modules) {
            ...
            for (ModuleEntrySystemPart se : m.getSystemElements()) {
                ...
                Node associationProvider =
                        imc.utilities.NodeUtils.getChildByName(inPlatform, "associationProvider");
                ...
                for (int i = 0; i < nl.getLength(); i++) {
                    ...
                    if (!nl.item(i).getNodeName().equals("associationType"))
                        continue;
            ...
            for (ModuleEntrySystemPart p : i2cProvider) {
                ...
                for (ModuleEntrySystemPart se : m.getSystemElements()) {
                    if (se.getAssociation() != null && se.getAssociation().getUsername().equals(p)) {
                        ...
                        Node nPlatform =
                                Platform.getInstance().getNodeOfSystemname(se.getNameOfSystemelement());
                        ...
                        for (int i = 0; i < nl.getLength(); i++) {
                            ...
                            if ((nShare == null) == (nonShare == null)) {
                                imc.utilities.Log.internalError(se.getNameOfSystemelement()
                                        + " check i2cAdrDoesNotCollide needs ether attribute 'shareable' or 'nonShareable'");
                                return;
                            }
                            String adr;
                            if (nShare != null) {
                ...
                for (int i = 0; i < connectedDevices.size() - 1; i++) {
                    for (int j = i + 1; j < connectedDevices.size(); j++) {
                        ...
                        if (!connectedDevices.get(i).adr.equals(connectedDevices.get(j).adr))
                            continue;
                        // check sharable
                        if (connectedDevices.get(i).isSharable && connectedDevices.get(j).isSharable) {
                            // both devices must have the same name
                            if (connectedDevices.get(i).device.equals(connectedDevices.get(i).device))
                                continue;

                            // same address different device name
                            imc.utilities.Log.setLocation(m.getSourceFileName(),
                                    connectedDevices.get(j).se.getLine(), connectedDevices.get(j).se.getCol());
                            imc.utilities.Log
                            .error("I2C address of " + connectedDevices.get(j).device + " already used.");
                            ...
                            imc.utilities.Log.setLocation(m.getSourceFileName(),
                                    connectedDevices.get(j).se.getLine(), connectedDevices.get(i).se.getCol());

                            imc.utilities.Log.error("I2C address " + connectedDevices.get(j).adr + " of '"
                                    + connectedDevices.get(j).se.getUserName() + "' is already used");
                            Log.note(m.getSourceFileName(), connectedDevices.get(i).se.getLine(),
